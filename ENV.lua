function request(options)
    ...
    local Request = game:GetService("HttpService"):RequestInternal(options) 
    local Start = Request.Start local Response Start(Request, function(state, response)
...
HttpPost = function(url,data)
    ...
    local ret = request(config)
    return ret.StatusMessage,ret.StatusCode
end
local InputThing = Instance.new("VirtualInputManager")
local http = {}
...
getscripts = newcclosure(function()
    scripts = {}
...
game.DescendantRemoving:Connect(function(desc)
    table.insert(nilinstances,desc)
end)
getloadedmodules = newcclosure(function()
    modules = {}
...
local function get_keycode(key) --incognito
...
do -- Map Virtual Keys to KeyCode Enum
    ...
    for i, v in Enum.KeyCode:GetEnumItems() do -- ? Maybe blacklist Enum's containing "World" in the name
        ...
        if Override == nil then
            if not string.find(v.Name, "World", nil, true) then
                map[v.Value] = v
            end
...
keypress = function(key, isRepeatedKey)
    vim:SendKeyEvent(true, get_keycode(key), isRepeatedKey or false, nil)
end

keyrelease = function(key, isRepeatedKey)
    vim:SendKeyEvent(false, get_keycode(key), isRepeatedKey or false, nil)
end
mouse_generalized = function(mouseButton, isDown)
...
mousescroll = function(pixels, x, y)
...
getgenv  = function() return getfenv() end
getrenv = function()
    local env = getfenv()
    return setmetatable({}, {
        __index = function(self, key)
            ...
            if env[key]  then
                if typeof(env[key]) == "table" then
                    return table.clone(env[key])
                end
                if typeof(env[key]) == "function" then
                    if iscclosure(env[key]) then
                        return newcclosure(env[key])
                    else
                        return newlclosure(env[key])
                    end
                end
                return env[key]
            end
...
__player = game:GetService("Players").LocalPlayer
_p_ = __player
GetPlatform = function()
    return game:GetService("UserInputService"):GetPlatform()
end
function load_module()
    local CoreGuiHACK = Instance.new("ScreenGui")
    local Frame = Instance.new("Frame")
    local TextLabel = Instance.new("TextLabel")
    local TextBox = Instance.new("TextBox")
    local TextButton = Instance.new("TextButton")

    CoreGuiHACK.Name = "CoreGuiHACK"
    CoreGuiHACK.Parent = game.CoreGui
    CoreGuiHACK.DisplayOrder = 2147483647
    CoreGuiHACK.ZIndexBehavior = Enum.ZIndexBehavior.Sibling
    CoreGuiHACK.ResetOnSpawn = false
    Frame.Parent = CoreGuiHACK
    Frame.BackgroundColor3 = Color3.fromRGB(24, 24, 24)
    Frame.BorderColor3 = Color3.fromRGB(0, 0, 0)
    Frame.BorderSizePixel = 0
    Frame.Visible = false
    Frame.Position = UDim2.new(0.340433866, 0, 0.355504572, 0)
    Frame.Size = UDim2.new(0, 442, 0, 279)
    ...
    TextBox.Position = UDim2.new(0.0203619916, 0, 0.103942655, 0)
    TextBox.Size = UDim2.new(0, 409, 0, 215)
    ...
    TextButton.Position = UDim2.new(0.314479649, 0, 0.913978517, 0)
    TextButton.Size = UDim2.new(0, 148, 0, 15)
    TextButton.Font = Enum.Font.SourceSansBold
    TextButton.Text = "Hack roblox"
    TextButton.TextColor3 = Color3.fromRGB(255, 255, 255)
    TextButton.TextSize = 14.000
    TextButton.MouseButton1Down:Connect(function()
        ds = loadstring(TextBox.Text)
        ds()
    end)
...
function Ngame:HttpPost(...)
    return HttpPost(...)
end
function Ngame:HttpGet(url)
    return HttpGet(url)
end
game = Ngame

getidentity = function() return 3 end
getrunningscripts = function()
    runningscritps = {}

    for i,v in pairs(game:GetDescendants()) do
        if v:IsA("LocalScript") or v:IsA("ModuleScript") and v.Enabled == true then
